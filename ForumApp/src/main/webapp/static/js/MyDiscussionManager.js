$(function () {
    ...
    var myDiscussionController = new MyDiscussionController($('#discussionContainer'));
    myDiscussionController.loadData();

    function MyDiscussionController(_container){
        var self = this;
        this.container = _container;

        this.loadData = function() {
            this.container.html("");
            $.get('../api/discussion/my', (json) => {
                if(json.length === 0){
        ...
        this.createNode = function(d){
            ...
            var topicNode = $('<span/>').addClass('text-secondary float-end small').text("Topic id: " + d.topicId);
